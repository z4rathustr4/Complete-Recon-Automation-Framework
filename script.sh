...
cat $domain/httpx.txt | nuclei -t cves -c 50 -o $domain/nuclei_scan/cves.txt
cat $domain/httpx.txt | nuclei -t vulnerabilities -c 50 -o $domain/nuclei_scan/vulnerabilities.txt
cat $domain/httpx.txt | nuclei -t misconfiguration -c 50 -o $domain/nuclei_scan/misconfiguration.txt
cat $domain/httpx.txt | nuclei -t technologies -c 50 -o $domain/nuclei_scan/tech.txt
...
cat $domain/vulnerabilities/openredirect/fuzzredirect.txt | openredirex -p ~/tools/openredirex/payloads.txt --keyword FUZZ | tee $domain/vulnerabilities/openredirect/confrimopenred.txt
echo -e ${GREEN}"\n[+] Searching For XSS"
...
clear
echo -e ${CPO}"\n[+] Querying crt.sh"
curl -s "https://crt.sh/json?q=$domain" | jq -r '.[].name_value' | sed 's/\*\.//g' | sort -u | tee $domain/domain_enum/crt.txt
sleep 10
...
echo -e ${CP}"\n[+] Nuclei Scanner Started:- "
cat $domain/final_domains/httpx.txt | nuclei -t cves -c 50 -o $domain/nuclei_scan/cves.txt
cat $domain/final_domains/httpx.txt | nuclei -t vulnerabilities -c 50 -o $domain/nuclei_scan/vulnerabilities.txt
cat $domain/final_domains/httpx.txt | nuclei -t misconfiguration -c 50 -o $domain/nuclei_scan/misconfiguration.txt
cat $domain/final_domains/httpx.txt | nuclei -t technologies -c 50 -o $domain/nuclei_scan/tech.txt
echo -e ${CPO}"\n[+] Collecting URLS:- "
...
echo -e ${ORANGE}"\n[+] Searching For Open Redirection:- "
cat $domain/gf/redirect.txt | qsreplace FUZZ | tee $domain/vulnerabilities/openredirect/fuzzredirect.txt
openredirex -l $domain/vulnerabilities/openredirect/fuzzredirect.txt -p ~/tools/OpenRedireX/payloads.txt --keyword FUZZ | tee $domain/vulnerabilities/openredirect/confrimopenred.txt
echo -e ${GREEN}"\n[+] Searching For XSS:- "
cat $domain/gf/xss.txt | kxss  | tee $domain/vulnerabilities/xss_scan/kxss.txt
cat $domain/vulnerabilities/xss_scan/kxss.txt | awk '{print $9}' | sed 's/=.*/=/' | tee $domain/vulnerabilities/xss_scan/kxss1.txt
cat $domain/vulnerabilities/xss_scan/kxss1.txt | dalfox pipe | tee $domain/vulnerabilities/xss_scan/dalfoxss.txt
